...
def register():
    ...
    try:
        cursor.execute(
            "INSERT INTO users (first_name, last_name, email, phone_number, password, user_type) VALUES (%s, %s, %s, %s, %s, %s)",
        ...
        if e.errno == 1062:
            data = {"status": "register error Email already exists"}
...
def addProperty():
    ...
    cursor.execute(
        "INSERT INTO properties (title, description, location, area, bedrooms, bathrooms, price, near_by, seller_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
def updateProperty(id):
    ...
    cursor.execute(
        "UPDATE properties SET title = %s, description = %s, location = %s, area = %s, bedrooms = %s, bathrooms = %s, price = %s, near_by = %s WHERE id = %s",
...
# ====== Get Seller Details
@app.route('/api/sellers/<string:seller_id>', methods=['GET'])
def showSellerDetails(seller_id):
    cursor.execute("SELECT * FROM users WHERE id = %s", [seller_id])
    sellerDetails = cursor.fetchall()
    if sellerDetails is None:
        return jsonify({"status": "Properties not there"}), 404
    else:
        return jsonify(sellerDetails), 200
